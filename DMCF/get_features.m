function [feature_pixels, currDim] = get_features(image, features, gparams, fg_size )
...
if gparams.cell_size == -1
    ...
    if (cf.fparams.useForColor && colorImage) || (cf.fparams.useForGray && ~colorImage)
        [feature_pixels, ~] = cf.getFeature(image,cf.fparams,gparams);
    end;
else
...
%     support_sz = [im_height, im_width];
     if currDim~= 32
        feature_pixels = reshape(feature_pixels, [size(feature_pixels, 1)*size(feature_pixels, 2), size(feature_pixels, 3)]);
    end
end
